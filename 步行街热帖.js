...
class Widget extends Base {
   ...
   */
  async renderSmall () {
    let res = await this.httpGet('https://hotrss.top/feed/hupu.json')
    let data = res['items']
    let topic = data[0]
    // 显示数据
    let w = new ListWidget()
    w = await this.renderHeader(w, 'https://w1.hoopchina.com.cn/images/pc/old/favicon.ico', '步行街热帖')
    let body = w.addStack()
    let title = body.addText(topic['title'])
    title.font = Font.boldSystemFont(15)
    w.url = this.actionUrl('open-url', topic['fileId'])
    w.addSpacer()
    return w
  }
  /**
  ...
  async renderMedium (count = 2) {
    ...
    for (let i = 0; i < count; i ++) {
      ...
      number.textColor = new Color("#c01e2e", 1)
      ...
      let detail = info.addText(topic['author']['name'])
