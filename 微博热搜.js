...
class Widget extends Base {
  ...
  async renderMedium (count = 2) {
    ...
    for (let i = 0; i < count; i ++) {
      ...
      number.textColor = new Color("#f26d5f", 1)
      ...
      let title_part = info.addStack()
      title_part.layoutHorizontally()

      let title = title_part.addText(topic['desc'])
      title.lineLimit = 1
      title.font = Font.lightSystemFont(15)

      if (topic['icon']) {
        let icon = title_part.addImage(await this.getImageByUrl(topic['icon']))
        icon.imageSize = new Size(20, 20)
      }

      info.addSpacer(5)

      let detail = info.addText(topic['desc_extr'].toString())
  ...
  async actionSetting () {
    ...
    a.addAction((arg==="0"?"✅ ":"")+"自带浏览器")
    a.addAction((arg==="1"?"✅ ":"")+"微博客户端")
    a.addAction((arg==="2"?"✅ ":"")+"国际版微博")
  ...
  async actionOpenUrl (url) {
    ...
    if (settings['type']==="0"){
      ...
      Safari.open('weibointernational://searchall?q=' + encodeURIComponent(k))
    }
  }
}
