...
const Running = async (Widget, default_args = "") => {
  ...
  if (config.runsInWidget) {
    ...
    if (!act || !M['_actions']) {
      ...
      const _actions = [
        async () => {
          Safari.openInApp("https://www.zhihu.com/people/ourongxing", false)
        }
      ]
...
class Widget extends Base {
   ...
   */
  async renderSmall () {
    let res = await this.httpGet('https://api.bilibili.com/x/web-interface/ranking/v2?rid=' + this.fenqu[this.settings['type']].rid)
    let data = res['data']
    let topic = data[0]
    // 显示数据
    let w = new ListWidget()
    w = await this.renderHeader(w, 'https://www.bilibili.com/favicon.ico', 'B站排行榜 / ' + this.fenqu[this.settings['type']].name)
    let body = w.addStack()
  ...
  async renderMedium (count = 2) {
    ...
    for (let i = 0; i < count; i ++) {
      ...
      number.textColor = new Color("#00abdf", 1)
      ...
      let detail = info.addStack()
      detail.layoutHorizontally()
      detail.centerAlignContent()

      let play = detail.addText((topic['stat']['view']/10000).toFixed(1).toString() + "万 |")
      play.font = Font.lightSystemFont(11)
      play.textColor = new Color('#959fb2', 1)
      play.rightAlignText()

      let name = detail.addText(" " + topic['owner']['name'])
      name.font = Font.lightSystemFont(11)
      name.textColor = new Color('#959fb2', 1)
      name.leftAlignText()


      bodyLeft.addSpacer(10)
      dom.url = this.actionUrl('open-url', topic['bvid'])
    }
    body.addSpacer()
  ...
  async actionSetting () {
    ...
    const arg = settings["type"] || "0"
    let a = new Alert()
    a.title="设置分区"
    a.message="设置想要展示的分区"
    this.fenqu.forEach((item, index) => {
      a.addAction((arg===index.toString()?"✅ ":"")+item.name)
    })
  ...
  async actionOpenUrl (url) {
    Safari.open('bilibili://video/' + url)
  }
